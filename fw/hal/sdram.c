...
void SDRAM_Init()
{
	...
	SDRAM->SDCR[0] = FMC_SDCR_SDCLK_2 | FMC_SDCR_RBURST;
	SDRAM->SDCR[1] = FMC_SDCR_NC_8 | FMC_SDCR_NR_12 | FMC_SDCR_MWID_16 | FMC_SDCR_NB_4 | FMC_SDCR_CAS_3;
	// Initialization step 2
	SDRAM->SDTR[0] = TRC(7) | TRP(2);
	SDRAM->SDTR[1] = TMRD(2) | TXSR(7) | TRAS(4) | TWR(2) | TRCD(2);
	// Initialization step 3	
	while(SDRAM->SDSR & FMC_SDSR_BUSY);
	SDRAM->SDCMR = FMC_SDCMR_MODE_CCE | FMC_SDCMR_CTB2 | (1 << 5);
	// Initialization step 4
	for(int i = 0; i  < 1000000; i++);
	// Initialization step 5
	while(SDRAM->SDSR & FMC_SDSR_BUSY);
	SDRAM->SDCMR = FMC_SDCMR_MODE_PALL | FMC_SDCMR_CTB2 | (1 << 5);
	// Initialization step 6
	while(SDRAM->SDSR & FMC_SDSR_BUSY);
	SDRAM->SDCMR = FMC_SDCMR_MODE_AUTOREFRESH | FMC_SDCMR_CTB2 | (8 << 5);
	// Initialization step 7
	while(SDRAM->SDSR & FMC_SDSR_BUSY);
	SDRAM->SDCMR = FMC_SDCMR_MODE_LOADMODE | FMC_SDCMR_CTB2 | (1 << 5) | (0x231 << 9);
	// Initialization step 8
	while(SDRAM->SDSR & FMC_SDSR_BUSY);
	SDRAM->SDRTR |= (0x0683 << 1);
	while(SDRAM->SDSR & FMC_SDSR_BUSY);

	// Clear the WriteProtect bit
	SDRAM->SDCR[1] &= ~FMC_SDCR_WP;
}
...
static void SetupGPIO()
{
	...
	//control pins
	GPIO_AFConf(GPIOB, 5, 12);
	GPIO_AFConf(GPIOB, 6, 12);
	GPIO_AFConf(GPIOC, 0, 12);
